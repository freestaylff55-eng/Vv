...
def check_banned_status(player_id):
    ...
    url = f"https://french-rosejellyfish.onpella.app/TEAM_Z7F_check_ban/{player_id}"
    try:
...
def newinfo(uid):
    try:
        url = f"https://french-rosejellyfish.onpella.app/TEAM_Z7F_info/{uid}"
        response = requests.get(url)
...
def get_random_avatar():
        avatar_list = [
        '902000011', '902000087', '902000267', '902000194', '902000066', 
        '902000017', '902000141', '902000151', '902000305', '902000084', 
        '902000018', '902000154', '902000152', '902000306', '902000306','902000091','902000208','902000209','902000210','902000211','902047016','902047016','902000347'
    ]
...
class FF_CLIENT(threading.Thread):
   ...
   #DexX
    ...
    def send_team_message(self, message_text):
        ...
        header_lenth_final = dec_to_hex(header_lenth)
        if len(header_lenth_final) == 2:
            final_packet = "1315000000" + header_lenth_final + self.nmnmmmmn(packet)  # 13 بدلاً من 12 للفريق
        elif len(header_lenth_final) == 3:
...
[FFA500َ][c][b]مرحبًا بك عزيزي المستخدم:
...
كل هذا متوفر تواصل معي !
 ...
 telegram:@حط التلجرام تبعك
 
[b][i][A5E2CFٍ] Dev: حط اسمك """, uid
                    )
                )
            ...
            if "1200" in data.hex()[0:4] and b"/sp" in data:
                try:
                    # استخراج البيانات من الرسالة أولاً
                    json_result = get_available_room(data.hex()[10:])
                    parsed_data = json.loads(json_result)
                    uid = parsed_data["5"]["data"]["1"]["data"]
                    ...
                    if len(command_split) > 1:
                        ...
                        def send_spam_invite():
                            try:
                                for i in range(50):  # إرسال 50 طلب
                                    ...
                                    if (i + 1) % 10 == 0:
                                        clients.send(
                                            self.GenResponsMsg(
                                                f"[C][B][00FF00]✅ تم إرسال {i + 1} طلب من أصل 50", uid
                                            )
                                        )
...
╰━━─[FFFF00]─╯
              [FFFF00]❖━━━╮
        [00FF00]لاغ عبر تيم كود:
              [00BFFF]/ُlag <team cod>
╰━━─[FFFF00]─╯""",uid
                            )
                       ) 
                        ...
                        clients.send(
                                    self.GenResponsMsg(
                                        f"""     [C][B]         [FFFF00]❖━━━╮
        [00FF00]لاغ متوسط عبر تيم كود:
              [00BFFF]/lag <team cod> 2
╰━━─[FFFF00]─╯
              [FFFF00]❖━━━╮
        [00FF00]لاغ قوي عبر تيم كود:
              [00BFFF]/ُlag <team cod> 3
╰━━─[FFFF00]─╯ """,uid
                            )
                       ) 
                ...
                try:
                    ...
                    if len(command_parts) < 2:
                        ...
                        try:
                            ...
                            clients.send(
                                self.GenResponsMsg(
                                    f"[C][B][00FF00]✅ تم الانضمام الخفي بنجاح!\n"
                                    f"[C][B][FF1493]👻 AlliFF_BOT GHOST MODE\n"
...
if __name__ == "__main__":
    try:
        client_thread = FF_CLIENT(id="4118237915", password="E9C0E872CD3B95CE44D06B753BB648DDB1C417AE008E731561AB1464C69F1C93")
        client_thread.start()
    except Exception as e:
